  ...
  class Evaluator
    def self.exec(code_table)
      ...
      (0..).reduce(init_vm) do |vm, _|
        ...
        Evaluator.print_debug_log(vm, code_table) if $debug
        ...
        when /^closure@(\d+)@([\w,]*)/
          ...
          vm.current_stack_frame_stack_push(closure)
            .current_stack_frame_line_num_add(1)
        when /^send@(\d+)/
          Evaluator.exec_send(vm, code_table, argc: $1.to_i)
        when /^jumpif@(\d+)/
    ...
    def self.exec_send(vm, code_table, argc:)
    ...
    def self.print_debug_log(vm, code_table)
